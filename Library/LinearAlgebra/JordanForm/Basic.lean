...
/-!
# Jordan Normal Form — Top-Level Assembly

This file proves that every endomorphism of a finite-dimensional module over an algebraically
closed field admits a Jordan normal form.  The argument proceeds in three stages: the maximal
generalized eigenspaces give an internal direct-sum decomposition of the whole space; the
restriction to each eigenspace, shifted by the eigenvalue, is nilpotent and therefore has a
Jordan basis by `nilpotent_has_jordan_basis`; these per-block bases are glued into a global
ordered basis by `glue_maxgen_jordan_blocks`, yielding a block-diagonal Jordan matrix for T.
-/
...
/-- The maximal generalized eigenspaces of `T` form an internal direct sum decomposition of `V`.
This is the key structural fact for Jordan form: over an algebraically closed field the
generalized eigenspaces are independent and their supremum is the whole space. -/
theorem max_gen_eigenspace_is_internal
  ...
  exact (DirectSum.isInternal_submodule_iff_iSupIndep_and_iSup_eq_top
    ...
    ⟨Module.End.independent_maxGenEigenspace T, Module.End.iSup_maxGenEigenspace_eq_top T⟩
...
/-- `T` maps each maximal generalized eigenspace into itself, i.e., each such subspace is
`T`-invariant, which allows forming the restriction `T.restrict` to each block. -/
theorem maxgen_eigenspace_invariant
    {K : Type*} [Field K] [DecidableEq K]
    {V : Type*} [AddCommGroup V] [Module K V] [FiniteDimensional K V]
    (T : V →ₗ[K] V) :
    ∀ μ : K, ∀ x ∈ (Module.End.maxGenEigenspace T μ : Submodule K V),
      T x ∈ (Module.End.maxGenEigenspace T μ : Submodule K V) := fun μ x hx => Module.End.mapsTo_maxGenEigenspace_of_comm (Commute.refl T) μ hx

/-- The restriction of `T` to the μ-generalized eigenspace, shifted by `−μ · id`, is nilpotent.
This is the analytic core of Jordan theory: by definition of the maximal generalized eigenspace,
`(T − μ · id)` is eventually zero on it, making its restriction nilpotent. -/
...
-- Built from: (1) `nilpotent_restrict_sub_smul` — N is nilpotent (deep maxGenEigenspace fact);
...
-- proof rewrites (N + μ•1) back to S via `abel`.
/-- Given that every nilpotent endomorphism on each generalized eigenspace has a Jordan basis
with zero diagonal, the restriction of `T` to that eigenspace also has a Jordan basis (with
eigenvalue `μ` on the diagonal), obtained by shifting the nilpotent part back by `μ · id`. -/
...
-- Assembly: glue per-eigenspace Jordan bases into a global one.
...
/-- Given an internal direct-sum decomposition of `V` into generalized eigenspaces and a
nilpotent Jordan basis hypothesis, assembles a global ordered basis for `V` in which the
matrix of `T` is in Jordan normal form. -/
...
-- Assembled from three proved pieces:
...
/-- Every endomorphism of a finite-dimensional module over an algebraically closed field admits
a Jordan normal form: there exists an ordered basis in which the matrix of `T` is block-upper-
triangular with Jordan blocks on the diagonal. -/
