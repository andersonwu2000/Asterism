...
# These tests replace `subprocess.run`, so no real `lake` ever spawns.
...
def test_lake_build_modules_chunks_and_aggregates(monkeypatch, tmp_path):
    calls: list[list[str]] = []
    monkeypatch.setattr(_lake.subprocess, "run", _fake_run_factory(calls))
    mods = _mods(600)
...
def test_lake_build_modules_timeout_reports_the_chunk(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_lake.subprocess, "run", fake_run)
