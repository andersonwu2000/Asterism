...
_WATERMARKS = {
    ...
    "Tooling/core/dispatcher/triggers.py": 721,  # +11 2026-09-05 the review seat waits on the group's in-flight batch, on T0/T1's own predicate  # +20 2026-09-04 reconcile_stuck_states class 3: an unanswered `Theorize` has a WORKER and nothing else re-derives it  # +11 2026-09-04 a Theorist row addressed to a group that already left is spent, not delayed  # +36 2026-09-03 `strategist_has_nothing_to_deliver` — the residual-case read of the classifier that door uses  # 2026-08-31 bench seat guard  # +18 2026-08-31 active-group seat guard + moot-verdict extinguish  # +suppress_stall (promotion gate) 2026-08-30
    ...
    "Tooling/state/transitions.py": 2090,  # +10 2026-09-07 only verify writes strategy 'succeeded': the async promotion gate gave "proposed, every sub-goal proved" a second meaning and the backstop settled the batch under an unproved goal — conscious bump  # +32 2026-09-06 the cascade's Theorist arm splits the infra road off: count the death, re-queue under the bound, settle with the pipeline's own headline once it is spent — conscious bump  # +28 2026-09-05 `_enter_pending_review` — the one road into review, which settles the Inject that produced the goal, plus T2's in-flight-batch hold  # +14 2026-09-04 the cascade's Theorist backstop: a worker that died of an exception settled nothing  # +6 2026-09-04 the Theorist layer: a Theorize is a batch delta  # +55 2026-09-04 goal status `dead` retired (v51): terminal vocabulary section + `_propagate_wrong_context` and the ordering it makes explicit  # +21 2026-09-02 HID §1.3: the shelve cascade splits into a read half (`shelve_cascade_targets`, what the confirm window names) and a write half  # +5 2026-08-30 problem terminal `refuted`  # +7 2026-09-02 v48 HID §3.2: a human ConfirmShelve is never a live promise (_awaiting_promised_batch)
    ...
    # `state/programme.py` outgrew the default cap 2026-09-07 with
    # the named-brick parser (`parse_bricks` / `Brick` / the `Uses:`
    # cycle check) and the `bricks` store beside it — the Programme
    # store is where both belong, since the bricks ARE the `## Proof`.
    ...
    "Tooling/state/db/problems.py": 1522,  # +5 2026-09-07 named bricks (v54): `open_batch_steps` carries the step's brick name  # +2 2026-09-05 the pending-review selector says WHY the batch suppression belongs to its callers  # +45 2026-09-04 `null_theorize_redispatch_specs` — the theory layer's half of the NULL-decision redispatch (a lost Theorist queue row was unrecoverable)  # +39 2026-09-04 the Theorist layer: `_theorize_in_flight` — the fourth in-flight arm, for a step that produces a document  # +57 2026-09-03 `_NOT_HUMAN_OPENED` — a Delegate a PERSON filed hands over no line, so the group it opened is not the parent's in-flight work and does not freeze its routine clock (owner ruling; union_closed 691 idle 2h)  # +9 2026-09-03 the batch-report ratchet takes `>=`: a batch settling inside the commit's own clock tick used to lose the strict comparison and reach no wake  # +145 2026-09-03 `open_batch_steps` / `batch_has_running_step` — the one place that splits an outcome-NULL batch step into running vs parked (SP7 phantom "still running" batches)  # +9 2026-09-03 `unacknowledged_inject_batches` also honours the carry-over mark (a mid-debate batch nobody acted on is still owed its report)  # born 2026-08-29 from the db.py split
    "Tooling/state/db/reach.py": 200,  # born 2026-08-29 from the db.py split
    "Tooling/state/db/strategies.py": 460,  # +35 2026-09-07 only verify writes strategy 'succeeded': the async promotion gate gave "proposed, every sub-goal proved" a second meaning and the backstop settled the batch under an unproved goal — conscious bump  # +25 2026-09-06 `decision_infra_deaths` / `record_decision_infra_death` — the bound on re-queueing a request whose worker died on infra, charged in ONE place — conscious bump  # born 2026-08-29 from the db.py split
    ...
    "Tooling/state/db_migrations.py": 3460,  # +48 2026-09-07 only verify writes strategy 'succeeded': the async promotion gate gave "proposed, every sub-goal proved" a second meaning and the backstop settled the batch under an unproved goal — conscious bump  # +17 2026-09-07 v54 named bricks (v54): `strategist_decisions.brick_name` — conscious bump  # +15 2026-09-06 v53 `strategist_decisions.infra_deaths` — conscious bump  # +125 2026-09-04 the Theorist layer (v52): five CHECK widenings on one live-DDL regex channel  # +93 2026-09-04 goal status `dead` retired (v51): `_migrate_to_v51` — the dead→shelved rewrite plus the CHECK-narrowing rebuild  # +13 2026-09-03 v50 report_carried_at (an additive column, but since v15 those ship as a version step)  # +30 2026-09-02 `_disarm_foreign_keys`: v48's FK disarm was a silent no-op inside the backfill's transaction, so no populated disk could migrate  # 2026-08-31 v47 benched  # +1 2026-08-30 last_words column  # +39 2026-08-30 v46 problems.state CHECK gains 'refuted'  # +routine audit 2026-08-30
    ...
    "Tooling/core/cli/problems.py": 1000,  # born 2026-08-28 from the cli.py split (A3)  # +10 2026-09-03 HID §3.9: a seeded paper id may sit on another Project's shelf, so init copies it onto this one before binding — conscious bump  # +20 2026-09-03 the wipe ends with a schema-DERIVED sweep over the problem-keyed tables (human_commands + routine_verdicts were never in the hand list and killed reset on the FK) — conscious bump  # +10 2026-09-03 the Goal/Strategy pipeline kinds get the dead_attempts "hangs off its pipeline" sweep the Problem/Group kinds already had (the target_id=0 failure sentinel matched neither) — conscious bump
    "Tooling/core/cli/diagnose.py": 850,  # born 2026-08-28 from the cli.py split (A3)
    "Tooling/core/cli/maint.py": 685,  # +81 2026-09-06 `theorize-freeze-adopt` — stamp a Theorist attempts dir frozen by hand with its resume point, so the next dispatch continues the document instead of re-authoring it — conscious bump  # +14 2026-09-04 goal status `dead` retired (v51): `asterism reject` files its own inject outcome  # 2026-08-31 bench/unbench commands  # born 2026-08-28 from the cli.py split (A3)  # +8 2026-09-03 `papers-migrate` — a thin printer over `papers/migrate.py`, which holds the whole move
    "Tooling/core/cli/main.py": 495,  # +25 2026-09-06 `theorize-freeze-adopt` parser (--decision, and the --author-sid help that says where a claude session id is findable) — conscious bump  # +10 2026-09-06 `lab run standard` — the reserved `standard` experiment named in --help, and `--seats seat=provider/model` (repeatable, the one knob a standard set is re-run to move) — conscious bump  # +30 2026-09-06 `lab` parser (snapshot/build/run/gc + the no-default-root rule in --help, which is where an operator meets it) — conscious bump  # +30 2026-09-03 `carry export` / `carry import` parser (one problem's whole state between workspaces) — conscious bump  # +12 2026-09-03 HID §3.9: `papers-migrate` parser + the Project argument paper-add/paper-index now take  # +3 2026-09-02 --scope help names the explicit-list form (HID §1.4)  # +3 2026-09-01: serve --host flag  # 2026-08-31 bench/unbench parsers  # +10 2026-08-30 catalog-verify subcommand  # born 2026-08-28 from the cli.py split (A3)
    ...
    "Tooling/pipeline/strategist/model.py": 326,  # +20 2026-09-07 named bricks (v54): `brick_name_of` and why `brief_field` keeps the retired `proof` mapping (it is the refusal route)  # +6 2026-09-05 `PROMPT_ALIAS` / `prompt_kind` — the one resolution from trigger_kind to prompt file, now that pending_review.md has retired  # born 2026-08-28 from the strategist.py split (B1)
    # The theory layer (theory_wake_design.md §3), born 2026-09-04.
    # One entry per stage, the `cleanup/` rule: the verdict parser is
    ...
    "Tooling/pipeline/theorist/__init__.py": 490,  # +115 2026-09-06 checkpoint + resume: `run_theorist` marks its phase at every state change and picks a re-dispatched request up where it stopped (a reviewer's quota death used to make the next author rewrite the whole document) — conscious bump  # +25 2026-09-06 an infra death is the framework's, not the request's (owner ruling): `_fail_spawn` holds the row open under the re-issue bound, and the run carries a quota-park budget for its review rounds — conscious bump  # +20 2026-09-06 a refused theory document lands too (owner ruling): one landing call for both roads, and the rejection's outcome carries the path, the fired criteria and the rigour flag  # +80 2026-09-05 the three failure roads are told apart at the source (`_fail` takes its headline per road), a spawn that died after writing is reviewed rather than thrown away, and a dead seat is named from its stderr like every other pipeline's
    "Tooling/pipeline/theorist/checkpoint.py": 530,  # born 2026-09-06: `_theorize.json`, the theory layer's resume point — its phases, the reconcile that reads the DIR over the field, the dir search/carry, and the predicate both cleanup sweeps ask before deleting a dir
    "Tooling/pipeline/theorist/verdict.py": 325,  # +25 2026-09-06 a refused theory document lands too (owner ruling): `fired_criteria` / `rigour_is_defective` / `RIGOUR_DEFECTIVE` — citability follows criterion 2, in one spelling for all three surfaces
    "Tooling/pipeline/theorist/review.py": 295,  # +25 2026-09-06 a refused verdict resumes the reviewer that wrote it (the judge's `479fd579` one seat over): the sid + retry_context state the loop now carries, and the refusal message it authors — conscious bump  # +25 2026-09-06 the reviewer absorbs the PROVIDER's failures (`_spawn`: quota park + the batch judge's infra budget) so its rc never costs the author's document — conscious bump  # +15 2026-09-06 a refused theory document lands too (owner ruling): the reviewer's dynamic manifest says what `agent/` now holds, since the static prompt calls it the accepted ones
    "Tooling/pipeline/theorist/landing.py": 235,  # +10 2026-09-06 `resumed from:` in the header — which run holds the earlier rounds' transcripts is archaeology the pipelines row cannot give  # +75 2026-09-06 a refused theory document lands too (owner ruling): the header opens with `status:` and carries the fired ruling, the filename carries `_rejected`, and `_free_name` stops a same-minute twin overwriting the record — conscious bump
    "Tooling/pipeline/strategist/verify.py": 1340,  # +110 2026-09-07 named bricks (v54): the per-Inject name gate (mint collision, target slug, the retired `proof`) plus `verify_injects`, the batch-level rules that need the `## Proof` AND the record — conscious bump; the SPLIT this file has been owed since 08-28 is still owed  # +45 2026-09-04 the Theorist layer: `Theorize`'s two required fields and the one-per-group in-flight gate  # 1150→1200 2026-09-02: the Ingest report gate checks the four paper headings (owner ruling) — conscious bump  # +32 2026-09-02 HID D2: the Ingest-report structural gate (unconditional since the wake prompts ask for the field)  # +9 2026-08-30 two-part brick shape gate on Inject proof  # +17 2026-08-30 refuted needs the gate-born brick; Ingest accepts a disproved root  # born 2026-08-28 from the strategist.py split (B1)  # +routine audit 2026-08-30; +7 2026-08-30 action gate reads target_id, shelved roots not in flight (3e61beb9 shipped red here — caught 05:59Z)
    "Tooling/pipeline/strategist/commit.py": 1168,  # +8 2026-09-07 named bricks (v54): an Inject writes its brick's NAME, and the human channel's prose keeps its legacy home  # +67 2026-09-04 the Theorist layer: `_commit_theorize` — the open batch step and the Theorist it dispatches  # +3 2026-09-04 goal status `dead` retired (v51): the Inject force-reopen comment now says why `disproved` stays in the list  # +9 2026-09-03 the acknowledgment law runs before the clock bump (`batch_ack.settle`; the law itself is a new module)  # +15 2026-09-02 HID D2: the Ingest report stored + rendered on the problem-terminal path  # +39 2026-09-02 HID §3.2/§3.3: `actor` threaded through every INSERT + the human Inject's dispatch band  # +11 2026-08-30 Ingest on a disproved root closes as refuted  # born 2026-08-28 from the strategist.py split (B1)
    "Tooling/pipeline/strategist/wake.py": 1030,  # +20 2026-09-07 named bricks (v54): the package returns its parsed bricks and the wake runs `verify_injects` on them — conscious bump, and the SPLIT is still owed  # 1000→1010 2026-09-06: the corrective turn re-reads whatever its rc, and an infra rc is named so the cascade re-queues the wake instead of charging it — conscious bump, and the SPLIT below is still owed  # 941→1000 2026-09-04: the `native_decide` soft confirm gate (one round-free bounce before the judge) + the judge/rebuttal warning split — conscious bump; SPLIT before the next one (the round loop wants its own module)  # +5 2026-09-03 the wake hands its commit the batch roster it DELIVERED  # 918→936 2026-09-03: the rebuttal names which files are fresh + the per-round delta pack (the pack itself is rendered in `round_materials`) — conscious bump  # 906→918 2026-09-03: the author's round-fresh record — `round_materials.refresh` into the attempts dir at every rebuttal round (the new module holds the rendering) — conscious bump  # +6 2026-08-30 last-words turn before the adversarial discard  # born 2026-08-28 from the strategist.py split (B1)
    ...
    "Tooling/agent/phase2_context/outcomes.py": 1200,  # +150 2026-09-07 named bricks (v54): `_brick_ledger_lines` — what became of every brick the batch NAMED, injected and used, since a `Uses:` brick is dispatched by nobody and the Strategist has no other way to learn whether a worker ever declared it — conscious bump  # +7 2026-09-06 a refused theory document lands too (owner ruling): the theory step's refused reading names the record it left  # +13 2026-09-05 an empty promise queue / batch scoreboard says "none" instead of vanishing  # 950→1030 2026-09-04: a step's landing is named from the goal's own status — `_landing_phrase` / `_landing_suffix` / `_alias_twin`, one vocabulary for the inline scoreboard and BATCHES.md (the judge read `landed:` over `(status=shelved)`); conscious bump  # +52 2026-09-04 the Theorist layer: the theory step's own reading on the batch scoreboard and in BATCHES.md  # +44 2026-09-03 the third reading of an unfinished step: a group the OWNER opened runs, but not on the reader's line — `_bucket` / `_owner_opened_lines`, one classifier for the scoreboard and the companion  # born 2026-08-28 from the phase2_context.py split (B2)
    "Tooling/agent/phase2_context/compile.py": 1360,  # +82 2026-09-04 the theory read: the trigger vocabulary the compile now CHECKS, the request in `## Trigger`, and the Programme as a pointer  # +6 2026-09-03 HID §1.2: the owner's notes roster rides beside the paper section on every Strategist wake  # +12 2026-09-03 the decision replay names a PARKED step instead of calling it dispatched  # +31 2026-08-31 heading demotion + wake-time TREE refresh  # +5 2026-08-30 the author's last words after the rebuttal  # +1 2026-08-30 refuted names the gate brick  # +41 2026-08-30 rebuttal surface after a discarded cycle (last round inline, REJECTED.md lazy)  # born 2026-08-28 from the phase2_context.py split (B2)  # +routine audit 2026-08-30
