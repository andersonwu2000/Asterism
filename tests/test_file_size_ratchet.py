...
_WATERMARKS = {
    ...
    "Tooling/pipeline/theorist/checkpoint.py": 520,  # born 2026-09-06: `_theorize.json`, the theory layer's resume point — its phases, the reconcile that reads the DIR over the field, the dir search/carry, and the predicate both cleanup sweeps ask before deleting a dir
    ...
    "Tooling/pipeline/theorist/review.py": 270,  # +25 2026-09-06 the reviewer absorbs the PROVIDER's failures (`_spawn`: quota park + the batch judge's infra budget) so its rc never costs the author's document — conscious bump  # +15 2026-09-06 a refused theory document lands too (owner ruling): the reviewer's dynamic manifest says what `agent/` now holds, since the static prompt calls it the accepted ones
