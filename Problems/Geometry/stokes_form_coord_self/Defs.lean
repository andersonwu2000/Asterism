/-
  Defs.lean — shared vocabulary for the `formCoordChange_self` obligation
  (§A.1 of the Stokes form-bundle construction). Just the genuine, sorry-free
...
/-- Transition map of `⋀ᵏ T*M`: precompose alternating maps by the (swapped)
    tangent transition. (Genuine sorry-free DATA; the coherence facts about it are
    the obligations.) -/
