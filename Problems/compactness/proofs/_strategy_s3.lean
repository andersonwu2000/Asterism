...
import Problems.compactness.proofs.L_s3_main_sub_4
...
theorem s3_main : ∀ {α : Type} (S : Set (PropForm α)),
  ...
  intro α S hS
  obtain ⟨M, hSM, hMfinsat, hMmax⟩ := s3_main_sub_1 S hS
  have hneg : ∀ p : PropForm α, PropForm.neg p ∈ M ↔ p ∉ M :=
    s3_main_sub_2 M hMfinsat hMmax
  have hconj : ∀ p q : PropForm α, PropForm.conj p q ∈ M ↔ (p ∈ M ∧ q ∈ M) :=
    s3_main_sub_3 M hMfinsat hneg
  obtain ⟨v, hv⟩ := s3_main_sub_4 M hneg hconj
