...
∀ {f : ℂ → ℂ} {z₀ : ℂ} {r : ℝ},
  ...
  (∮ z in C(z₀, r), f z) = 2 * Real.pi * Complex.I *
    Complex.residue f z₀
...
- `Mathlib.MeasureTheory.Integral.CircleIntegral` — `circleIntegral` API,
  with the parser notation `∮ z in C(z₀, r), f z` (the literal tokens
  `C(`, `,`, `)` are part of the notation declaration).
- `Complex.residue` (introduced via `Defs.lean`) — formal Laurent
  coefficient `a₋₁`; Mathlib does not yet provide a complex-analytic
  residue, so the project-local definition stands in.
...
2. **Term-by-term integration over the circle**. For `k ≠ -1`,
   `∮ z in C(z₀, r), (z − z₀)^k = 0` (antiderivative exists in the
   annulus). The `k = -1` term gives `2πi`.

3. **Swap sum and integral**. Uniform convergence of the Laurent series
   on the circle of radius `r` justifies the interchange.
