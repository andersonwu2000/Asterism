...
-- integrand_interval_integrable_step_wrapper: wraps analytic_integrand_interval_integrable
-- proof inline (direct sibling citation unavailable from Builder patch.lean).
theorem integrand_interval_integrable_step_wrapper
    {U : Set ℂ} {g : ℂ → ℂ} {γ : ℝ → ℂ}
    (hU : IsOpen U)
    (hg : AnalyticOn ℂ g U)
