...
-- Pointwise dispatch z ≠ a: combine continuity of Q at z (from analyticity on
-- the punctured set) with the local segment-integral identity
-- F(z+h) - F z = ∫₀¹ Q(z + t·h) · h dt (specialised from h_segment) to extract
-- the little-o estimate giving HasDerivAt F (Q z) z.
-- Sub-goals:
--   (1) continuous_at_of_analytic_on_punctured — Builder: AnalyticOn on an
--       open punctured set ⇒ ContinuousAt at each point.
--   (2) hasderivat_from_segment_integral — Backward: the analytic core
--       (ContinuousAt Q z + local segment ID on Metric.ball z (dist z a)
--       ⇒ HasDerivAt F (Q z) z) via uniform continuity of Q on a small ball.
theorem s10532
    {Q : ℂ → ℂ} {a : ℂ}
  ...
  have h_seg : ∀ h : ℂ, ‖h‖ < dist z a →
      F (z + h) - F z = ∫ t in (0:ℝ)..1, Q (z + (t : ℂ) * h) * h :=
    h_segment z hz
  exact hasderivat_from_segment_integral z hz h_cont h_seg
