...
-- family_chain: Jordan chain relation for each block — case split on inl (last vs castSucc)
-- and inr (complement basis vector in ker N); uses Fin.lastCases, hv_chain, hv_top, hx.
-- entry_kind: Builder
theorem family_chain
  ...
  · -- Sum.inl ⟨t, ht⟩: chain block, t : Fin p, ht : 0 < l t, j : Fin (l t + 1)
    ...
    | cast i =>
      -- j = i.castSucc: inner chain element, use hd t i
      rw [hv_chain ⟨t, ht⟩ i]
      rcases hd t i with h0 | ⟨i', hi'_eq, hi'_val⟩
      · left
