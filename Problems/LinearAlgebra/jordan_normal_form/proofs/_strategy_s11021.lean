...
import Problems.LinearAlgebra.jordan_normal_form.proofs.L_sum_decomp_to_neg

namespace Problems.LinearAlgebra.jordan_normal_form

-- Pure sum bookkeeping: hg's `∑ i, g i • v i = 0` splits along the sigma+sum index
-- into a chain-block sum and a complement-block sum; habove kills above-bottom chain
-- terms; hv_chain/hv_C identify the surviving terms as `d⟨t.1,0⟩` resp. `cb c`. Then
-- `eq_neg_of_add_eq_zero_left` flips `chain + comp = 0` into `comp = -chain`.
theorem s11021
  ...
  exact sum_decomp_to_neg p m l (fun s => (↑(d s) : W)) (fun c => (↑(cb c) : W))
