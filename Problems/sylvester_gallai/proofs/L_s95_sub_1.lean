import Mathlib
import Problems.sylvester_gallai.Defs

namespace Problems.sylvester_gallai

-- entry_kind: Builder
theorem s95_sub_1 :
    ∀ (P : Finset (ℝ × ℝ)),
      (∃ a ∈ P, ∃ b ∈ P, ∃ c ∈ P, ¬ Collinear a b c) →
      (∀ p ∈ P, ∀ q ∈ P, p ≠ q → ∃ r ∈ P, Collinear p q r ∧ r ≠ p ∧ r ≠ q) →
      ∀ p ∈ P, ∀ q ∈ P, ∀ r ∈ P, p ≠ q → ¬ Collinear p q r →
      ∀ s ∈ P, Collinear p q s → s ≠ p → s ≠ q →
      ((p.1 - r.1) * (q.1 - p.1) + (p.2 - r.2) * (q.2 - p.2)) *
          ((s.1 - r.1) * (q.1 - p.1) + (s.2 - r.2) * (q.2 - p.2)) ≥ 0 →
      ((s.1 - r.1) * (q.1 - p.1) + (s.2 - r.2) * (q.2 - p.2))^2 ≤
        ((p.1 - r.1) * (q.1 - p.1) + (p.2 - r.2) * (q.2 - p.2))^2 →
      (q.1 - p.1)^2 + (q.2 - p.2)^2 > 0 := by
  intro P _ _ p _ q _ r _ hpq _ s _ _ _ _ _ _
  rcases eq_or_ne p.1 q.1 with h1 | h1
  · rcases eq_or_ne p.2 q.2 with h2 | h2
    · exact absurd (Prod.ext h1 h2) hpq
    · have hne : q.2 - p.2 ≠ 0 := sub_ne_zero.mpr (Ne.symm h2)
      have hpos : (q.2 - p.2) ^ 2 > 0 := by positivity
      nlinarith [sq_nonneg (q.1 - p.1)]
  · have hne : q.1 - p.1 ≠ 0 := sub_ne_zero.mpr (Ne.symm h1)
    have hpos : (q.1 - p.1) ^ 2 > 0 := by positivity
    nlinarith [sq_nonneg (q.2 - p.2)]

end Problems.sylvester_gallai
