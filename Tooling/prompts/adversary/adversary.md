...
- `proposal.md` — the CANDIDATE revision under judgment: `# Title` (this batch's goal), `## Argument` (why the requirement needs this plan), `## Proof` (every brick as `Theorem.` statement then `Proof.` argument — no gaps), `## Roadmap` (opens with `Relation:` — the statement this Programme works toward and how it stands to the MAIN claim; then three bands — PAST closures, one per bullet / NOW this batch's decisions, one per bullet / AHEAD a one-line brief then a numbered plan of the next steps, naming the load-bearing difficulty and where it is attacked; closures name the exact dead instantiation and a self-producible restart condition).
...
2. **Relation**: `proposal.md`'s ## Roadmap must state what this Programme works toward and how it stands to the MAIN claim — implies it, is equivalent to it, reduces it to a smaller claim, or is a condition whose failure refutes it — argued, not asserted. A wrong direction (work whose success would not move the MAIN claim), a Programme that names its load-bearing difficulty but attacks it nowhere (attacked = a NOW brick bites it, or the Proof carries an argument or a counterexample on it; a name in AHEAD is not an attack), a route re-walked unchanged, or a contradiction of a verified Programme record, is not allowed.
...
{"criteria": {
   ...
   "2": ["clear: <the statement this Programme works toward and its relation to the MAIN claim> — <where the load-bearing difficulty is attacked>"],
...
No criterion takes a bare `clear` — every clear carries one concrete sentence of why it holds for THIS proposal. Criterion 2's reason IS the naming: the statement worked toward, its relation to the MAIN claim, and where the load-bearing difficulty is attacked.
...
- When you see the author settling for a cheap substitute, circling the known, or patching along a wrong route: name the load-bearing wall the Programme is avoiding and require the revision to face it — to push the theory one step further there, toward the claim or toward refuting it; both count, avoidance does not.
