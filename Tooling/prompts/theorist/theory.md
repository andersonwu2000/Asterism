...
3. `## Load-bearing work` — what this document is an advance on, and the open statement: the one whose proof or refutation settles the claim, named exactly, and the attempts made on it here — a new or invented route among them — the arguments tried and where each one stops, the reformulations considered, the special cases or constructions examined, and what they showed. An attempt that fails and says precisely why is a result; an open statement that is only named is not.
