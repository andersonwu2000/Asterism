...
1. **Worth**: the document must advance on the present state — a concept not already landed, not in the notes, not in the cited literature — and must say how the theory stands to the MAIN claim (implies it, is equivalent to it, reduces it to a smaller claim, a condition whose failure refutes it, and so on), with the relation argued or proved. Restating the record, reproducing the notes in new words, a wrong direction (a statement whose truth would not move the MAIN claim), or a relation resting on something the record has not proved and the document does not prove, is not allowed.
2. **Rigour**: every statement claimed proved must have a complete and rigorous proof — **re-derive the proofs**, and reproduce the computations the plan leans on. A gap papered over, or a check you cannot reproduce, is not allowed.
3. **Load-bearing work**: the document must name the open statement exactly — the one whose proof or refutation settles the MAIN claim — and must have tried it, a new or invented route among the attempts: an argument carried to the point where it stops, a reformulation weighed, a case or construction worked through, with what each showed. An open statement that is only named, or attempts that are only announced, is not allowed. Attempts are judged on their substance, not their success: a failed attempt that says precisely why it fails passes this criterion.
4. **Leads**: every conjecture and next step must carry its argument and the test it survived (a case analysis, a construction, a computation), and say what a counterexample would have to look like. A conjecture with no reason and no test, or a next step that is a wish, is not allowed.
...
{"criteria": {
   ...
   "3": ["clear: <the open statement as the document names it, and the attempt that actually bites>"],
...
- When the author circles the known or patches along a wrong route: name the open statement the document avoids and require the revision to face it — one step further there, toward the claim or toward refuting it; both count, avoidance does not.
- Write the file in the shape above; a file in another shape comes back to you once for rewriting.
