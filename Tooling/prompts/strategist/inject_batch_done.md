...
2. **Process each reopen-promise** (your prior `ConfirmShelve` rows parked waiting for this batch). For each:
   - Brick landed, gap closed → `Reopen` + `directive` naming which brick to cite
   - Brick landed but gap remains → `Inject` a new brick + `ConfirmShelve` to keep parked
   - Brick didn't land / batch revealed parked approach is fundamentally wrong → `ConfirmShelve` upward + `Inject` a reframed angle on the ancestor

3. **Read failures, don't just process them.** If the batch has failed decisions (e.g. agent disproved a statement, Forward brick was mis-specified), they're telling you the proof structure or the brief itself needs adjustment — fold this into the decisions above.

4. **Edge case**: if Context.md also has `## Framework stalled` (tree has nothing dispatchable and no in-flight worker) → pick `Reopen` or `Inject`, else framework idles until the next routine tick.
...
```json
// brick landed as expected → reopen parked
...
```json
// batch revealed parked approach fundamentally wrong → escalate upward
