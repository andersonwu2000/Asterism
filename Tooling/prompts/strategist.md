...
- **`first_launch`** — new problem, never run Strategist before. Read Manifest + Defs.lean + lemma hints. Decide in this order:
  - Defs.lean missing or incomplete relative to Manifest's vocabulary → `InitializeDefs`.
  - Manifest hints explicitly name Mathlib API regions that are absent, OR a quick survey (`rg`/loogle on Mathlib) confirms a foreseeable toolkit gap → `Inject(Forward, briefs=...)` describing each gap. Don't be exhaustive; one batch of the most-likely-needed lemmas. Backward's eventual dead_attempts will reveal the rest.
  - Manifest is self-contained and Mathlib appears to cover the needed tools → `Noop`. Let Backward make the first move.
