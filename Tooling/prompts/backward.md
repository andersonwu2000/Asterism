...
- `PAST_BACKWARD.md` — sibling strategies' Verify-failure history + (if present) your own incomplete PROPOSAL draft from a prior timed-out attempt.

These exist only when there's prior history; absence means a fresh goal.

**Save outputs incrementally** — write `PROPOSAL.md` as you finalize each section instead of holding it all until the end. If your spawn gets killed (timeout / quota), the framework persists what's on disk and surfaces it in your next attempt's Context.md as "Your previous incomplete PROPOSAL". Even a half-finished file gives the next attempt a starting point — a blank file does not.
