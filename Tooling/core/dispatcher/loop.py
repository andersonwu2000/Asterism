...
def run(workspace: Path, *, once: bool = False,
    ...
    # `dispatch.ram_budget` set, the ledger paces the two worlds BENEATH
    # `pool_size` — Lean admission follows the gateway's ledger-driven
    # open-slot count and NL admission follows measured available RAM (NL
    # priority: a wake surge may shrink the Lean field; claimed slots
    # finish first). `pool_size` stays a hard ceiling over both.
    ledger = None
        ...
        while not (stopping or drifting or quota_waiting
            ...
            if ledger is None:
                ...
                if len(futures) >= pool_size:
                    ...
                    # (owner 2026-09-06). The ledger prices RAM, but the
                    # binding resource on a subscription board is the
                    # five-hour quota window: 2 Strategists + 2 Theorists
                    # + 17 Formalizer spawns at full ledger width burned
                    # one out in 3.5h (2026-09-05 20:30Z). RAM was being
                    # used as an indirect pacer; this is the direct one.
                    break
