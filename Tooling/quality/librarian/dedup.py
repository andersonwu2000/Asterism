...
def file_cleanup_polish(workspace: Path, problem: str, target_file: str,
    ...
    for attempt in range(max_retries + 1):
        ...
        warns = _polish_warnings(
            _build_with_output(workspace, new_text, prefix="_polish_warn")[1])
...
def file_cleanup_audit(workspace: Path, problem: str, target_file: str,
    ...
    for attempt in range(max_retries + 1):
        ...
        rc = agent.spawn_llm(kind="librarian", prompt_path=prompt_path,
                             problem_dir=problem_dir, attempts_dir=attempts,
                             session_id=agent.new_pipeline_id())
        out = attempts / _AUDIT_OUTPUT
        if rc != 0 or not out.exists():
            prev_error = f"no {_AUDIT_OUTPUT} was produced"
            continue
        new_text = out.read_text(encoding="utf-8")
        side = attempts / _AUDIT_RENAMES
        renames = _valid_renames(
            _coerce_renames(_json_loads_or_none(
        ...
        for d, fqn_after in zip(decls_in_file, fqns_after):
            ...
            for old, new in renames.items():     # sibling types may cite renamed
                expected, _ = replace_token(expected, old, new)
        ...
        warns = _polish_warnings(
            _build_with_output(workspace, new_text, prefix="_audit_warn")[1])
        if best is None or len(warns) < best[0]:
