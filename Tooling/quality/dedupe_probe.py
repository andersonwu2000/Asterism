...
def partition_header_errors(output: str, module_at_line: "dict[int, str]",
    ...
    for m in _ERR_LINE_RE.finditer(output):
        ...
        if cm:
            mod = cm.group(1)
            if mod in modules and mod not in deferred and mod not in dropped:
                deferred.append(mod)
            continue
...
def import_batches(workspace: Path, modules: "list[str]",
    ...
    todo = list(dict.fromkeys(modules))
    depth = 0
    while todo and depth < _MAX_ROUNDS:
        ...
        for _round in range(_MAX_ROUNDS):
            ...
            rc, out = run(content)
            dropped, deferred = partition_header_errors(out, at)
            ...
            cur = [m for m in cur if m not in dropped and m not in deferred]
            for mod in deferred:
        ...
        todo = deferred_next
    for mod in todo:
        dropped_all.setdefault(mod, "could not be housed within the round budget")
