...
| Var | Effect | Default |
|---|---|---|
| `ASTERISM_VERIFY_RETRY` | When Verify Step-1 lake build fails, ask LLM ONCE to rewrite the strategy patch (sub-goals untouched). Set `0` to disable. | `1` (on) |
...
| `ASTERISM_CLAUDE_ALLOWED_TOOLS` | Replace the per-spawn computed allowlist (problem-dir + mathlib subtree). Empty string (`""`) drops the flag. | derived from `req.problem_dir` |
...
| `ASTERISM_BUILDER_PROVIDER` / `ASTERISM_BACKWARD_PROVIDER` | Per-kind provider override. | follows `ASTERISM_LLM_PROVIDER` |
