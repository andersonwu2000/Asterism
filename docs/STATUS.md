...
9. **Phase 1 `tactic_try` 18 個 tactic 跑成 18 次完整 lake build** — `pipeline/__init__.py:507-510` for-loop sequentially 寫 body / `_lake_build` 每個 tactic 獨立 invocation。Lake 每次啟動 ~5-15s，全 18 失敗 ≈ 90-270s 純 startup 浪費。**改用 Lean 內建 `by first | t1 | t2 | …`**：單一 lake build 內 elaborator 試所有 tactic 短路，獨立啟動成本只付一次，預估 SG 級問題每次省 ~150s × 15 個 tactic_try_exhausted ≈ ~38 min（baseline 4h16m 的 ~15%）。代價：失去 per-tactic forensic granularity（目前 `won_<tac>.lean` 記成功的 tactic 名），改成 `won_first_block.lean` 單一 snapshot 即可。order 不變（cheap rfl/decide first，expensive exact?/aesop last） — `first` 短路保留同樣 prefer-cheap-first 行為。改動範圍：~10 行替換現有 for-loop。
