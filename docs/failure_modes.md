...
- `strategist_no_delta` — the mechanical delta gate: three consecutive revision turns left `proposal.md` byte-identical to the body the Adversary last rejected (the revision never reached the file — the signature of a broken revision turn, not of author conviction). Identical resubmissions are bounced back to the author with a mechanical notice instead of re-judged; the third bounce discards like a rejection (same audit trail, target cooldown, no attempts++)
